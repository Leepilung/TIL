...
while True:
    ...
    if menu == Menu.머리에노드삽입:
        lst.add_first(int(input('머리 노드에 넣은 값을 입력하시오 . : ')))

    elif menu == Menu.꼬리에노드삽입: 
        lst.add_last(int(input('꼬리 노드에 넣을 값을 입력하시오. : ')))
        ...
        pos = lst.search(int(input('검색할 값을 입력하시오. : ')))
        if pos >= 0:
        ...
        print('그 값의 데이터는 포함되어' + (' 있습니다.' if int(input('판단할 값을 입력하시오. :')) in lst else '있지 않습니다.'))
