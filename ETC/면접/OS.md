...
`프로세스(Process)`는 `실행 중인 프로그램`으로 메모리에 적재되어 CPU의 할당을 받을 수 있는 것을 말한다.
...
`프로세스`는 `실행되는 프로그램 자체`와 `프로그램이 실행되는 주변 환경`까지 포함하는 개념입니다.
...
메모리를 직접 건드릴 수 있는 언어에서 정해진 크기보다 더 큰 데이터를 메모리에 쓰면서 의도하지 않은 메모리 영역에 에디터가 저장되는 것을 말한다.
...
이후에 해당 프로세스가 time quantom(타임 퀀텀)을 받아 다시 처리하게 되었을 경우 이전에 처리하던 정보를 바탕으로 프로세스가 시작되어야할 위치를 찾아서 동작을 이어갈 수 있게 해야한다.
