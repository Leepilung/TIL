...
프로미스가 아닌 애들마저 전부 예외처리가 가능하다.(`예외 처리 내용 보강 필요`)
...
-   콜백함수를 `태스크 큐`에 넣는 함수들 ( 함수 실행시 함수 하나 )
...
자바스크립트는 모든 선언(var, let, const, function, function*, class)을 호이스팅한다. 그러나 `var`로 선언된 변수와 달리 `let`의 경우 선언문 이전에 참조할면 참조 에러를 발생시킵니다. `let`으로 선언된 변수는 스코프의 시작에서 변수의 선언까지 일시적 사각지대(Temporal Dead Zone; TDZ)에 빠지기 때문입니다.
