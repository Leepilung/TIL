...
이로 인해 콜백 패턴의 사용이 가능하고 고차함수 생성이 가능하며 클로저, 메모이제이션 등이 가능합니다.
...
함수 객체에는 프로토타입이라는 특수한 유형의 객체 프로퍼티(속성)가 존재하는데 이러한 prototype 프로퍼티를 통해 생성자 함수는 인스턴스에게 프로토타입 객체에 있는 데이터, 메소드를 상속합니다.

예로 어떠한 인스턴스 객체의 key에 접근할 때, 해당 객체에게 key가 없다면 그 다음으로 상위 프로토타입(원형) 속성에서 key가 있는지 확인하고 만약 없다면 더 상위의 프로토타입(부모)ㅇ서 찾습니다. 이를 프로토타입 체인이라고 합니다.
