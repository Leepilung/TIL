...
4. 파일을 받아와서 우선 HTML 파일을 파싱 후, DOM(Document Object Model) 트리 구축 (Parsing)
...
콜백 함수란 `함수 안에서 실행되는 또 다른 함수`를 의미하며, `이벤트에 의해 호출되는 함수`입니다.
...
try catch구문을 활용해야 에러 핸들링이 가능합니다.

이를 이용하여 프로미스가 아닌 구문들 또한 전부 예외처리가 가능합다.(`예외 처리 내용 보강 필요`)
